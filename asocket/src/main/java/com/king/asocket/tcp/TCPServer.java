...
public class TCPServer
        implements ISocket<ServerSocket> {
        ...
        obtainExecutor().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    isConnected = true;
                    LogUtils.d(String.format("localAddress:%s:%d",
                    ...
                    while (isStart()) {
                        try {
                            LogUtils.d("TCPServer is started ...");
                            Socket socket = mServer.accept();
                            processSocketTask(mExecutor,
                                    socket);
    ...
    private void processSocketTask(Executor executor,
                                   final Socket socket) {
    ...
    private void processSocket(Socket socket) {
        try {
            String host = socket.getInetAddress()
                    .getHostAddress();
